...
* __core__ - модули, которые поддерживает основная команда разработчиков Ansible.
* __network__ - поддерживает Ansible Network Team.
* __certified__ - поддерживают партнеры Ansible
* __community__ - поддерживает сообщество Ansible
...
```
ansible-playbook 1_show_commands.yml -vvv
```
...
Задача будет выполнена только в том случае, если в выводе sh_ip_int_br_result.stdout будет найдена строка 4.4.4.4
```
when: "'4.4.4.4' not in sh_ip_int_br_result.stdout[0]"
```
