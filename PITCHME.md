...
Так как в режиме lines модуль сравнивает команды независимо друг от друга, он обнаружил, что не хватает только двух команд из трёх.
...
* Модуль обнаружил, что не хватает команды ```permit icmp any any``` и добавил её.
...
В начало ACL была добавлена строка permit udp any any:
```
ip access-list extended IN_to_OUT
 permit udp any any
 permit tcp 10.0.1.0 0.0.0.255 any eq www
...
То есть, последние 4 строки выглядят так, как нужно, и в том порядке, в котором нужно, но, при этом, есть лишняя строка.
...
Это значит, что при использовании ```match: exact```, важно, чтобы был какой-то способ удалить конфигурацию, если она не соответствует тому, что должно быть.
...
В такой же ситуации, при использовании ```match: exact```, было бы обнаружено изменение и ACL был бы изменён на состоящий только из строк в списке lines.
...
* __line__ - в этом режиме отправляются только те команды, которых нет в конфигурации. Это режим по умолчанию.
...
Попробуем запустить playbook 10_ios_config_replace_line.yml:
```yml
- name: Run cfg commands on router
  hosts: 192.168.100.1
...
Но, так как ACL сначала удаляется, а затем применяется список команд lines, получилось, что теперь ACL с одной строкой.
...
Этот параметр взаимоисключающий с lines (то есть, можно указывать или lines или src). Он заменяет модуль ios_template, который скоро будет удалён.
...
Запустим playbook 11_ios_config_src_jinja.yml:
```yml
- name: Run cfg commands on router
  hosts: cisco-routers
