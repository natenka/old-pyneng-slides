...
Информация берётся из таких команд:
* dir
* show version
...
* ansible_net_config - конфигурация (для команды Cisco show running-config)
...
* ansible_net_interfaces - словарь со всеми интерфейсами устройства. Имена интерфейсов - ключи, а значения - параметры каждого интерфейса
...
Для того, чтобы посмотреть, какие именно факты собираются с устройства, можно добавить флаг -v (на скриншоте информация сокращена):
```
...
Модуль copy позволяет копировать файлы с управляющего хоста (на котором установлен Ansible) на удалённый хост.
